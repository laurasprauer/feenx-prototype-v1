...
const dietaryPreferences: DietaryPreference[] = [
  ...
  { id: "ketogenic", title: "Ketogenic" },
  { id: "food-allergies", title: "Food Allergies" },
  { id: "no-restrictions", title: "No Restrictions" },
];
...
export default function OnboardingStep3() {
  ...
  const togglePreference = (preferenceId: string) => {
    setSelectedPreferences((prev) => {
      ...
      if (preferenceId === "no-restrictions") {
        return prev.includes("no-restrictions") ? [] : ["no-restrictions"];
      }

      // If selecting any other preference, remove "no-restrictions" if present
      const withoutNoRestrictions = prev.filter(
        (id) => id !== "no-restrictions",
      );

      return withoutNoRestrictions.includes(preferenceId)
        ? withoutNoRestrictions.filter((id) => id !== preferenceId)
        : [...withoutNoRestrictions, preferenceId];
    });
  };
  ...
  const gridPreferences = dietaryPreferences.filter(
    (pref) => pref.id !== "no-restrictions",
  );
  const noRestrictionsOption = dietaryPreferences.find(
    (pref) => pref.id === "no-restrictions",
  );
  ...
  return (
    <div className="min-h-screen bg-[#E0F1EB] relative">
        ...
        <div className="relative flex items-center justify-center">
          ...
          <div className="relative flex items-center justify-between w-full max-w-[656px]">
            ...
            <div className="w-10 h-10 rounded-full bg-white border-2 border-[#4FA587] flex items-center justify-center">
              <span className="text-[#4FA587] font-semibold text-sm">3</span>
            </div>
      ...
      <div className="w-full max-w-[656px] mx-auto px-4 pb-20">
        ...
        {/* No Restrictions Option - Centered */}
